const answerList = [
  ...
  ['салат', 'У оливье есть чувства! Холодец должен жить на воле!', -1],
  ['колбас', 'Сколько еще вы будете убивать докторов ради производства докторской колбасы?', -1],
  ['краб', 'Они воруют у крабов палочки так же, как они украли у меня детство!', -1],
  ...
  for (let x = 1; x < answers.length; x++) {
    if (question.includes(answers[x][0])) {
      ...
      answerList[x][2] % 2
        ...
        : 'Сколько можно повторяться? Ты осознаешь количество углекислого газа ты выдохнул в атмосферу за прошедшую бесцельно потраченную минуту?';
    }
  }
